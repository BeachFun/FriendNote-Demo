...
namespace Bitsplash.DatePicker
{
    ...
    {
        [Header("Настройка DatePickerDropDownBase")]
        /// <summary>
        /// 
        /// </summary>
        ...
        {
            DateTime? date = GetSelectedDate(); // get the selected date
            string t = NoSelectionPrompt;
            try
            {
                if (date.HasValue)
                    t = date.Value.ToString(labelDateFormat); // find the correct string to show for the selected date
            }
            catch (Exception)
            {
